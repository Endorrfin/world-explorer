...
scripts/
  ...
  generate_symbols.py         one-time generator: produced symbols.json + symbols_uk.json (195 entries each)
src/
  data/
    ...
    symbols.json              EDITABLE national symbols (animal/plant/dish, EN), keyed by ISO-2
    symbols_uk.json           EDITABLE national symbols (Ukrainian), keyed by ISO-2
  ...
  components/
    ...
    CountryDetail.tsx         detail panel (Known for / National symbols / People / Economy / Land / Neighbours)
    RecordsTab.tsx            "🏆 World Records" — 34 cards (14 computed + 20 static); static facts hand-maintained
    ...
    FlagMatchGame.tsx         "Match the flags" — connect 4 flags to 4 countries
    CompareGame.tsx           "Who's bigger?" — pick the larger country (play until wrong)
    SortGame.tsx              "Sort by size" — order 4 countries smallest → largest
    Quiz.tsx                  quiz hub: 9 world games (incl. the map game) + scoring
    UkraineQuiz.tsx           3-in-1 Ukraine quiz (oblast map / centres / fun facts)
...
```
data.xlsx ──build_data.py──► countries.json  (facts.json + facts_uk.json + symbols*.json merged in)
...
- **Area fix for micro-states** lives in `AREA_FIX` in `build_data.py`: the source rounds
  Vatican City's land area to `0 km²`, which dropped it from the "Smallest country" record
  (Monaco wrongly won). `AREA_FIX = {"VA": 0.44}` restores its real 44-hectare area so the
  record is correct. Re-run `npm run data` after editing it.
- **Static "World Records" facts** in `RecordsTab.tsx` are hand-maintained and can age — keep
  them current (e.g. Italy now has **61** UNESCO sites; South Africa has **12** official
  languages since 2023). Note "Most official languages" is a simplification: Zimbabwe (16)
  and Bolivia (37) recognise more.
...
- **v1.12** — **Crimea shown as Ukraine** (UN GA Res 68/262): `scripts/lib_crimea.mjs` moves
  ...
  a `theme-color` meta.
- **v1.13** — **National symbols**: a national animal, plant and dish for all 195 countries,
  bilingual EN/UA (`src/data/symbols.json` + `src/data/symbols_uk.json`, generated by
  `scripts/generate_symbols.py`). Merged into `countries.json` by `build_data.py` as
  `animal`/`plant`/`dish`/`animalUk`/`plantUk`/`dishUk`. Shown in CountryDetail as
  "🐾 National symbols" (between "Known for" and "People"), fully localised. Brand globe/name
  now navigates to home (Map tab). About tab updated to describe all new features.
- **v1.14** — **World Records in Explore + mobile nav polish**: a "🏆 World Records" section
  (34 cards — 14 computed from live data, 20 static geographic facts) embedded at the bottom
  of the Explore tab below the country grid, replacing the former separate Records tab (5 tabs
  instead of 6). Russia sorted last in every record list. On mobile, the nav bar,
  continent-zoom buttons and heatmap controls are now single-row horizontal-scroll strips
  instead of wrapping into multiple lines, freeing ~4 rows of screen space. Fixed author
  footer (Vasyl Krupka · LinkedIn) with `position: fixed` so it is always visible.
- **v1.15** — **more quiz games + record/data fixes + share assets**: the quiz hub now has
  **9 world games** — added "Match the flags" (`FlagMatchGame`), "Who's bigger?"
  (`CompareGame`) and "Sort by size" (`SortGame`) — plus a **3-in-1 Ukraine quiz**
  (`UkraineQuiz`: oblast map, regional centres, fun facts). Corrected three "World Records":
  Italy 58 → **61** UNESCO sites, South Africa 11 → **12** official languages, and "Smallest
  country" now resolves to **Vatican City** (new `AREA_FIX` gives its 0-rounded area its real
  0.44 km²). Added a LinkedIn share kit under `scripts/linkedin/` — a stat-card PNG, a
  map-hero PNG and an MP4, all rendered from `worldmap.json` with Pillow — and refreshed
  `linkedin-post.md` (EN + UA).

## Possible improvements (roadmap)

### UI / UX
- **Mobile/touch polish** — nav bar, continent pills and heatmap controls are now scrollable
  single rows (v1.14); still to do: tap target sizing, quiz button layout on small phones.
...
- Add fields kids enjoy: **languages, currency, time zone, calling code already shown,
  area/population rank, flag meaning** (national animal/plant/dish ✓ done v1.13).
...
- More modes: currency, language, flag → capital. ("Which country is bigger?" ✓ done v1.15 —
  "Who's bigger?" + "Sort by size".)
...
Carry these forward — they are the open risks/trade-offs as of v1.15.
